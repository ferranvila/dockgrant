...
module.exports = {
    ...
    /**
     * Checks if the path variable is absolute (starts with '/')
     * @param _path {string} Path value
     */
    checkPath: function (_path) {
        // TODO: Check for windows. Maybe use a node function
        if (!_path.startsWith('/')) {
            this.log('error', 'The path variable <' + _path + '> should be absolute');
            this.exit(1);
        }
    },
